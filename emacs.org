...
Search with consult and deadgrep:
...
- Go to node: =g G=
- Virtual index: =I=
* Interactive commands, universal argument and prefix system

Interactive commands are commands that are executed from the mini-buffer after we press `M-x`. These commands are special emacs lisp functions that may require user input (in contrast with regular functions that we pass arguments to when calling them). For example, `M-x describe-key` will prompt for the key to show help for.

Note on evil mode: the `C-u` that serves as "universal argument" (described below) for interactive commands in is "scroll up" in Evil mode.
 * An easy way to use the universal argument is to switch to the insert mode (then `C-u M-x ...` will add the universal argument.
 * Another alternative: temporary switch to emacs mode with `Ctrl-Z` (and one more `Ctrl-Z` to switch back to evil).

Besides user input, interactive commands are able to take extra arguments passed via Emacs prefix system.
...
The `C-u` on its own passes the default "universal argument" which usually tells the command "do what you usually do, but differently".

For example, `M-x run-lisp` starts the default lisp REPL and `C-u M-x run-lisp` will ask which REPL to run before starting it.
